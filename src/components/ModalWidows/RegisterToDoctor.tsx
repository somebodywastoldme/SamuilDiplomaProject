...
export interface IRegisterToDoctorProps extends IModalWindowProps {
}
...
const RegisterToDoctor: FC<IRegisterToDoctorProps> = (props) => {
    const [doctor, setDoctor] = useState<Doctor>(props.data as Doctor);
    ...
    return (
        <div>
            <DialogContentText style={{marginBottom: '8px'}}>
                Ви хочете записатися до лікаря <b>{doctor.user.firstName + ' ' + doctor.user.secondName}</b>
            </DialogContentText>
            <Grid
                container
                direction="column"
...
export default ModalWrapper(RegisterToDoctor);